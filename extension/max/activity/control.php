...
class activity extends control
{
     ...
     * @return void
     */
    public function browse($browseType = 'all', $param = '', $orderBy = 'order_desc', $recTotal = 0, $recPerPage = 20, $pageID = 1)
    {
        ...
        $actionURL  = $this->createLink('activity', 'browse', "browseType=bySearch&param=myQueryID");
    ...
    public function edit($activityID = 0)
    {
        ...
        if($_POST)
        {
            ...
            if(!empty($changes))
            {
                $actionID = $this->loadModel('action')->create('activity', $activityID, 'changed');
    ...
    public function assignTo($activityID = 0)
    {
        if($_POST)
        {
            ...
            die(js::locate($this->createLink('activity', 'browse'), 'parent'));
        }
