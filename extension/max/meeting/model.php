...
class meetingModel extends model
{
    ...
    public function minutes($meetingID)
    {
        ...
        $this->dao->update(TABLE_MEETING)->data($meeting)->autoCheck()->where('id')->eq($meetingID)->exec();

        if(!dao::isError())
        {
            $this->loadModel('file')->saveUpload('meeting', $meetingID, 'minutesFiles', 'minutesFiles');
    ...
    public function getListByUser($browseType = '', $orderBy = 'id_desc', $param = '', $pager = null)
    {
        ...
        return $this->dao->select('t1.*')->from(TABLE_MEETING)->alias('t1')
            ...
            ->andWhere("t2.deleted = '0' or t2.id IS NULL")
