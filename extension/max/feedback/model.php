...
class feedbackModel extends model
{
    ...
    {
        $oldFeedback = $this->dao->select('*')->from(TABLE_FEEDBACK)->where('id')->eq((int)$id)->fetch();
        ...
        $feedback = fixer::input('post')
            ...
            ->setDefault('module', 0)
            ->setDefault('product', 0)
            ->setIF($assignedTo != '' && $assignedTo != $oldFeedback->assignedTo, 'assignedDate', $now)
        ...
        if(!dao::isError())
        {
            $this->file->updateObjectID($this->post->uid, $id, 'feedback');
    ...
    public function batchUpdate()
    {
        ...
        foreach($data->titles as $feedbackID => $title)
        {
            ...
            $this->dao->update(TABLE_FEEDBACK)->data($feedback)
                ...
                ->exec();
            if(dao::isError()) return helper::end(js::error('feedback#' . $feedbackID . dao::getError(true)));
    ...
    public function getList($browseType = 'wait', $orderBy = 'id_desc', $pager = 'null', $moduleID = 0)
    {
        ...
        return $this->dao->select('t1.*,t2.dept')->from(TABLE_FEEDBACK)->alias('t1')
            ...
            ->beginIF($browseType == 'public' and $this->config->vision == 'lite')->andWhere('t1.public')->eq('1')->fi()
            ...
            ->beginIF($this->session->feedbackProduct != 'all')->andWhere('t1.product')->eq((int)$this->session->feedbackProduct)->fi()
            ->beginIF(!empty($modules))->andWhere('t1.module')->in($modules)->fi()
            ->beginIF($browseType != 'public' and $this->app->getMethodName() == 'browse')->andWhere('t1.openedBy')->eq($this->app->user->account)->fi()

            ->beginIF(!$this->app->user->admin and $browseType != 'openedbyme')
    ...
    public function getBySearch($queryID = 0, $orderBy = 'id_desc', $pager = null)
    {
        ...
        return $this->dao->select('t1.*,t2.dept')->from(TABLE_FEEDBACK)->alias('t1')
            ...
            ->andWhere('t1.status', true)->ne('clarify')
            ->orWhere('t1.openedBy')->eq($this->app->user->account)
            ->markRight(1)
            ->fi()
            ->orderBy($orderBy)
    ...
    public function getUserFeedbackPairs($account = '', $limit = 10, $appendID = '')
    {
        $feedbackPairs = $this->dao->select('t1.id, t1.title')->from(TABLE_FEEDBACK)->alias('t1')
            ...
            ->andWhere('t1.status')->ne('noreview')
            ->andWhere('t1.status')->ne('clarify')
            ->markRight(1)
    ...
    public function getFeedbackView($products)
    {
        return $this->dao->select('*')->from(TABLE_FEEDBACKVIEW)
            ->beginIF($products)->where('product')->in($products)->fi()
     ...
     * @return array
     */
    public function getGrantProducts($isPairs = true, $isDefault = false)
    {
        ...
        $stmt  = $this->dao->select('t1.*')->from(TABLE_PRODUCT)->alias('t1')
            ->leftJoin(TABLE_PRODUCT)->alias('t2')->on('t1.program=t2.id')
            ->where('t1.deleted')->eq('0')
            ...
            ->beginIF(!$admin)->andWhere('t1.id')->in($this->app->user->view->products)->fi()
    ...
    public function processActions($feedbackID, $actions)
    {
        ...
        $actions   = array();
        $orderList = $this->dao->select('id')->from(TABLE_ACTION)->where('id')->in(array_keys($totalActions))->orderBy('date, id')->fetchPairs('id');
     ...
     * @return string
     */
    public function printCell($value, $feedback, $users, $allProducts, $depts, $modulePairs, $viewMethod = 'view', $browseType = '', $stories = array(), $bugs = array(), $todos = array(), $tasks = array(), $tickets = array())
    {
        ...
        $feedbackLink = helper::createLink('feedback', $viewMethod, "feedbackID=$feedback->id&browseType=$browseType");
        ...
        case 'id':
            if($canBatchAction)
            {
                echo html::checkbox('feedbackIDList', array($feedback->id => '')) . html::a($feedbackLink, sprintf('%03d', $feedback->id));
            }
            else
            {
        ...
        case 'title':
            ...
            echo html::a($feedbackLink, $feedback->title);
            break;
        case 'status':
    ...
    public function buildOperateViewMenu($feedback)
    {
        ...
        if($this->config->vision == 'rnd' && strpos('closed|clarify|noreview', $feedback->status) === false)
        {
            $menu .= $this->buildMenu('feedback', 'reply', $params, $feedback, 'view', 'restart', '', "iframe", true, '', $this->lang->feedback->reply);

            if(self::isClickable($feedback, 'toStory') || self::isClickable($feedback, 'toUserStory') || self::isClickable($feedback, 'toTask') || self::isClickable($feedback, 'toBug') || self::isClickable($feedback, 'toTodo') || self::isClickable($feedback, 'toTicket'))
            {
                ...
                if(self::isClickable($feedback, 'toTicket'))
                {
                ...
                if(self::isClickable($feedback, 'toStory'))
                {
                ...
                if(self::isClickable($feedback, 'toTask'))
                {
                ...
                if(self::isClickable($feedback, 'toBug'))
                {
    ...
    public function buildOperateBrowseMenu($feedback)
    {
        ...
        if($adminMethod or ($this->app->getModuleName() == 'my' and $this->config->vision != 'lite'))
        {
            ...
            {
                $disabled = strpos('closed|clarify|noreview', $feedback->status) === false ? '' : ' disabled';
                ...
                $menu .= "<ul class='dropdown-menu pull-right c-actions text-center' role='menu' style='position: unset; min-width: auto; padding: 5px 0; white-space: nowrap;'>";
                $menu .= $this->buildMenu('feedback', 'suspend', $params, $feedback, 'pause', '', 'iframe btn-action btn', true, '');
                $menu .= $this->buildMenu('feedback', 'toStory', "product=$feedback->product&extra=fromType=feedback,fromID=$feedback->id", '', 'browse', $this->lang->icons['story'], '', ($iframe ? 'iframe' : '') . $disabled, $iframe, "data-width=95% {$disabled}", $this->lang->feedback->toStory);
                ...
                $menu .= $this->buildMenu('feedback', 'toTodo', "feedbackID=$feedback->id", '', 'browse', $this->lang->icons['todo'], '', ($iframe ? 'iframe' : '') . $disabled, $iframe, $disabled, $this->lang->feedback->toTodo);
    ...
    public function createFromImport()
    {
        ...
        foreach($data->title as $key => $value)
        {
            ...
            $feedbackData->module      = $data->module[$key];
    ...
    public function productSetting()
    {
        ...
        {
            if(!empty($data->feedbacks[$key]) or !empty($data->tickets[$key]))
            {
