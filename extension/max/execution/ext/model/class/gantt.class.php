...
class ganttExecution extends executionModel
{
    ...
    public function buildGanttItem($groupID, $task, $execution, $branches)
    {
        ...
        $showID     = $this->loadModel('setting')->getItem("owner=$account&module=execution{$execution->id}&section=gantt&key=showID");
        $showBranch = $this->setting->getItem("owner=$account&module=execution{$execution->id}&section=gantt&key=showBranch");
        if($showID == '') $showID = 1;
        if($showBranch == '') $showBranch = 1;
        ...
        if($task->branch and isset($branches[$task->branch]) and $showBranch) $name .= "<span class='label label-info'>{$branches[$task->branch]}</span> ";
        $name .= $task->name;
