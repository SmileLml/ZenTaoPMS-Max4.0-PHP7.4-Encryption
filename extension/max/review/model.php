...
class reviewModel extends model
{
    ...
    public function getList($projectID = 0, $browseType = '', $orderBy = '', $pager = null)
    {
        if($browseType == 'wait')
        {
            ...
            $reviews     = $this->getByList($pendingList, $orderBy, $pager);
        }
        else
        {
     ...
     * @return void
     */
    public function getByList($idList = array(), $orderBy = 'id_desc', $pager = null)
    {
        $reviews = $this->dao->select('t1.*, t2.version, t2.category, t2.product')->from(TABLE_REVIEW)->alias('t1')
            ->leftJoin(TABLE_OBJECT)->alias('t2')
            ->on('t1.object=t2.id')
    ...
    public function create($projectID = 0, $reviewRange = 'all', $checkedItem = '')
    {
        ...
        $review->doc         = $data->doc;
        $review->docVersion  = $doc ? $doc->version : 0;
        $review->status      = 'wait';
    ...
    public function saveResult($reviewID, $type = 'review')
    {
        ...
        if($type == 'review')
        {
            ...
            $this->loadModel('effort')->create('review', $reviewID, $result->consumed, $review->title, $approvalID);
        }
        else
        {
            $audit  = new stdclass();
            ...
            $this->loadModel('effort')->create('review', $reviewID, $result->consumed, $this->lang->review->audit . $review->title);
        }

        $action = $type == 'review' ? 'Reviewed' : 'Audited';
        $this->loadModel('action')->create('review', $reviewID, $action, $this->post->opinion, ucfirst($result->result));
        ...
        foreach($issueResult as $id => $result)
        {
            ...
            $issue->approval    = $currentApprovalID;
    ...
    public static function isClickable($review, $action)
    {
        ...
        if($action == 'submit')  return ($review->status == 'draft' || ($review->status == 'fail' && $review->result == 'fail'));
     ...
     * @return void
     */
    public function printCell($col, $review, $users, $products, $pendingReviews)
    {
