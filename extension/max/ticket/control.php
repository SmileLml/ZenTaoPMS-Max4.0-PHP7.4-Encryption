...
class ticket extends control
{
    ...
    public function browse($browseType = 'wait', $param = 0, $orderBy = 'id_desc', $recTotal = 0, $recPerPage = 20, $pageID = 1)
    {
        ...
        $this->loadModel('common')->saveQueryCondition($this->dao->get(), 'ticket');
    ...
    public function create($productID = 0, $extras = '')
    {
        ...
        $this->view->builds        = $this->loadModel('build')->getBuildPairs($productID, 'all', 'noreleased');
        $this->view->moduleID      = $moduleID;
        $this->display();
    }
    ...
    public function close($ticketID, $confirm = 'no')
    {
        ...
        {
            $changes = $this->ticket->close($ticketID);
            if(dao::isError()) return print(js::error(dao::getError()));
    ...
    public function batchEdit()
    {
        if(isset($_POST['ticketIDList']))
        {
            ...
            $this->view->products     = $this->loadModel('feedback')->getGrantProducts(true, true);
     ...
     * @return void
     */
    public function syncProduct($productID = 0)
    {
        echo $this->fetch('feedback', 'syncProduct', "productID=$productID&module=ticket");
    }
    ...
    public function export($browseType, $orderBy)
    {
        if($_POST)
        {
            ...
            /* Get tickets. */
            $sql = $this->dao->select('
                t1.id, product, module, title, status, pri, type, `desc`, openedBuild, deadline, assignedTo, mailto,
                keywords, startedBy, startedDate, feedback, openedBy, openedDate, activatedBy, activatedDate, activatedCount,
                resolvedBy, resolvedDate, resolution, closedBy, closedDate, closedReason, editedBy, editedDate,
                group_concat(t2.customer) as customer,group_concat(t2.contact) as contact,group_concat(t2.notifyEmail) as notifyEmail')
        ...
        $fileName = zget($this->lang->ticket->tabList, $browseType, '');
