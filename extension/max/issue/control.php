...
class issue extends control
{
    ...
    public function edit($issueID, $from = 'project')
    {
        ...
        $executionList = array(0 => '');
        $executions = $this->loadModel('execution')->getByProject($issue->project);
        foreach ($executions as $execution) $executionList[$execution->id] = $execution->name;
        ...
        $this->view->executions  = $executionList;
        $this->view->projectList = $this->project->getPairsByModel('all', 0, 'noclosed,multiple');
        $this->view->teamMembers = $this->project->getTeamMemberPairs($objectID);
    ...
    public function export($objectID, $from = 'project', $browseType = 'all', $orderBy = 'id_desc')
    {
        ...
        $browseType  = zget($this->lang->issue->labelList, $browseType, '');
