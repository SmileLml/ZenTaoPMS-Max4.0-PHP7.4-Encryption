...
class budgetModel extends model
{
    ...
    {
        return $this->dao->select('*')->from(TABLE_BUDGET)->where('project')->eq($projectID)->andWhere('deleted')->eq(0)->orderBy($orderBy)->page($pager)->fetchAll();
    }
    ...
    {
        return $this->dao->select('stage')->from(TABLE_BUDGET)->where('project')->eq($projectID)->andWhere('deleted')->eq(0)->orderBy('stage_asc')->fetchPairs();
    }
    ...
    {
        return $this->dao->select('subject')->from(TABLE_BUDGET)->where('project')->eq($projectID)->andWhere('deleted')->eq(0)->orderBy('subject_asc')->fetchPairs();
    }
    ...
    public function getSubjectStructure()
    {
        ...
        $structure    = array();
        $subjectPairs = $this->getSubjectPairs($this->session->project);
    ...
    {
        $budgets = $this->dao->select('*')->from(TABLE_BUDGET)->where('project')->eq($projectID)->andWhere('deleted')->eq(0)->fetchAll();
        $stages  = $this->getStages($this->session->project);
