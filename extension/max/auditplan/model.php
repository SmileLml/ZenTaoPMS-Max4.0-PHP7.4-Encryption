...
class auditplanModel extends model
{
    ...
    public function check($auditplanID, $projectID)
    {
        ...
        foreach($_POST['result'] as $listID => $result)
        {
            ...
            {
                if(!$_POST['comment'][$listID])
                {
                    dao::$errors[] = $this->lang->auditplan->commentEmpty;
                    return false;
                }
                /* Create NC*/
    ...
    public function batchCheck($projectID)
    {
        ...
        foreach($_POST['result'] as $auditplanID => $lists)
        {
            ...
            foreach($lists as $listID => $result)
            {
                ...
                if($result == 'fail' and $mode == 'normal')
                {
                    if(!$_POST['comment'][$auditplanID][$listID])
                    {
                        dao::$errors[] = $this->lang->auditplan->commentEmpty;
                        return false;
                    }
     ...
     * @return array
     */
    public function getResult($auditplan)
    {
        $results = $this->dao->select('*')->from(TABLE_AUDITRESULT)
            ->where('auditplan')->eq($auditplan->id)
