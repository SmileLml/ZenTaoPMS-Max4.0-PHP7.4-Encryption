...
class auditplan extends control
{
     ...
     * @return void
     */
    public function browse($projectID = 0, $from = 'project', $processID = 0, $browseType = 'all', $param = '', $orderBy = 'id_desc', $recTotal = 0, $recPerPage = 20, $pageID = 1)
    {
        ...
        $this->view->auditplans      = $this->auditplan->getList($projectID, $browseType, $param, $orderBy, $pager, $processID);
    ...
    public function create($projectID = 0, $from = 'project')
    {
        ...
        $executionList = array(0 => '') + $this->loadModel('execution')->getPairs($projectID, 'all', 'all');
    ...
    public function edit($auditplanID = 0, $from = 'project')
    {
        ...
        $executionList = array(0 => '') + $this->loadModel('execution')->getPairs($auditplan->project, 'all', 'all');
    ...
    public function batchCheck($projectID = 0, $from = 'project')
    {
        ...
        $draftResults = array();
        foreach($auditIDList as $auditID) $draftResults[$auditID] = $this->auditplan->getDraftResults($auditID);
    ...
    public function check($auditplanID, $projectID = 0)
    {
        ...
        $this->view->draftResults = $this->auditplan->getDraftResults($auditplanID);
    ...
    public function result($auditplanID)
    {
        ...
        $this->view->users     = $this->loadModel('user')->getPairs('noclosed|noletter');
        $this->view->auditplan = $auditplan;
        $this->view->results   = $this->auditplan->getResult($auditplan);
        $this->view->object    = $this->auditplan->getObjectName($auditplan->objectType, $auditplan->objectID);
        $this->view->checkList = $this->auditplan->getCheckList($auditplan->objectType, $auditplan->objectID);
        $this->display();
    }
