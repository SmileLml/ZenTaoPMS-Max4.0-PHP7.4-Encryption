        ...
        foreach($this->groupTasks as $groupKey => $tasks)
        {
            ...
            foreach($tasks as $taskKey => $task)
            {
                ...
                else
                {
                    ...
                    if($this->orderBy == 'status' || ($task->status != 'cancel' && $task->status != 'closed')) $groupLeft += $task->left;
                }
            ...
            foreach($tasks as $taskKey => $task)
            {
                ...
                foreach($this->excelKey as $key => $letter)
                {
                    ...
                    if($key == "group_{$this->orderBy}")
                    {
                        ...
                        $value  = $groupName;
                        $value .= "\n" . sprintf($this->lang->execution->groupSummaryAB, $groupSum, $groupWait, $groupDoing, $groupEstimate, $groupConsumed, $groupLeft);
