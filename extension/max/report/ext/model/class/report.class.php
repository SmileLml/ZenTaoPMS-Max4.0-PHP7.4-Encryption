...
class reportReport extends reportModel
{
    ...
    public function getWorkSummary($begin, $end, $dept, $type, $pager = null)
    {
        ...
        $users = $this->dao->select('DISTINCT t1.account')->from(TABLE_USER)->alias('t1')
            ->leftJoin(TABLE_TASK)->alias('t2')->on("t1.account = t2.$userField")
            ->where('t1.deleted')->eq(0)
            ->andWhere('t2.deleted')->eq(0)
            ->andWhere('t2.parent')->ge(0)
            ->beginIF($condition)->andWhere('t2.status')->in('closed, done')->fi()
            ->andWhere("t2.$dateField")->lt($end)
            ->andWhere("t2.$dateField")->ge($begin)
            ->beginIF($dept)->andWhere('t1.dept')->in($depts)->fi()
        ...
        $tasks = $this->dao->select('*')->from(TABLE_TASK)
            ...
            ->beginIF($condition)->andWhere('status')->in('closed, done')->fi()
            ->andWhere("$dateField")->lt($end)
            ->andWhere("$dateField")->ge($begin)
            ->beginIF($users)->andWhere("$userField")->in($users)->fi()
            ->orderBy('execution')
            ->fetchAll('id');
        ...
        foreach($tasks as $task)
        {
            ...
            if(isset($teams[$task->id]))
            {
                foreach($teams[$task->id] as $team)
                {
                    ...
                    {
                        $userTasks[$account][$task->execution][$task->id] = clone $task;
                    }
                    else
                    {
                        $userTasks[$account][$task->execution][$task->id]->estimate += $task->estimate;
                        $userTasks[$account][$task->execution][$task->id]->consumed += $task->consumed;
                        $userTasks[$account][$task->execution][$task->id]->left     += $task->left;
                    }
                }
            }
            else
            {
                $task->multiple = false;
                if($condition)  $userTasks[$task->finishedBy][$task->execution][$task->id] = $task;
                if(!$condition) $userTasks[$task->assignedTo][$task->execution][$task->id] = $task;
            }
        }
