...
{
    public function getList($projectID, $orderBy, $pager)
    {
    ...
    public function update($baselineID)
    {
        ...
        $baseline->data  = json_encode($data);
        $baseline->range = $range;
        $this->dao->update(TABLE_OBJECT)->data($baseline)->autoCheck()->batchCheck($this->config->cm->create->requiredFields, 'notempty')->where('id')->eq($baselineID)->exec();
        unset($baseline->data);
