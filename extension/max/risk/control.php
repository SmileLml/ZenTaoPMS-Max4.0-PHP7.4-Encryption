...
class risk extends control
{
    ...
    public function edit($riskID, $from = 'project')
    {
        ...
        $executionList = array(0 => '');
        $executions = $this->loadModel('execution')->getByProject($risk->project);
        foreach ($executions as $execution) $executionList[$execution->id] = $execution->name;

        $this->view->title        = $this->lang->risk->common . $this->lang->colon . $this->lang->risk->edit;
        $this->view->projectList  = $this->loadModel('project')->getPairsByModel('all', 0, 'noclosed,multiple');
        $this->view->risk         = $risk;
    ...
    public function view($riskID, $from = 'project')
    {
        ...
        $this->view->approvers    = $this->loadModel('assetlib')->getApproveUsers('risk');
        $this->view->libs         = $this->assetlib->getPairs('risk');
        $this->view->executions   = array(0 => '') + $this->loadModel('execution')->getPairs($risk->project);
    ...
    public function export($objectID, $browseType, $orderBy)
    {
        if($_POST)
        {
            ...
            /* Get executions. */
            $executions = $this->dao->select('id, name')->from(TABLE_PROJECT)->where('id')->in(array_column($risks, 'execution'))
                ->fetchAll('id');
