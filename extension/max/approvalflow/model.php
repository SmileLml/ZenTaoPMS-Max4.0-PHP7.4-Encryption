...
class approvalflowModel extends model
{
     ...
     * @return array
     */
    public function getList($type = 'all', $orderBy = 'id_desc')
    {
        $flows = $this->dao->select('*')->from(TABLE_APPROVALFLOW)
            ->where('deleted')->eq(0)
        ...
        foreach($nodes as $node)
        {
            ...
            else
            {
                ...
                else if(isset($node->reviewers) and !empty($node->reviewers))
                {
                    ...
                    if(!isset($node->reviewers[$index]->users) or empty($node->reviewers[$index]->users))
                    {
            ...
            else
            {
                ...
                if(isset($node->reviewers) and !empty($node->reviewers))
                {
                    foreach($node->reviewers as $reviewer)
                    {
                        ...
                        {
                            foreach($reviewer->users as $account) $result['appointees']['reviewer'][] = $account;
                            break;
                        }
                    }
                }
                if(isset($node->ccs) and !empty($node->ccs))
                {
                    foreach($node->ccs as $cc)
                    {
                        ...
                        {
                            foreach($cc->users as $account) $result['appointees']['ccer'][] = $account;
                            break;
                        }
                    }
                }

                if(count($result['types']) >= 1 or isset($result['appointees'])) $results[] = $result;
            }
        }
