...
class approvalflow extends control
{
     ...
     * @return void
     */
    public function browse($type = 'project', $orderBy = 'id_desc')
    {
        if($type == 'project' and !helper::hasFeature('waterfall')) $type = 'workflow';

        $uri = $this->app->getURI(true);
        $this->session->set('flowList', $uri, $this->app->tab);

        $flows = $this->approvalflow->getList($type, $orderBy);
