...
class approvalModel extends model
{
    ...
    public function getLastReviewDate($approvalID)
    {
        return $this->dao->select('reviewedDate')->from(TABLE_APPROVALNODE)
            ...
            ->andWhere('reviewedDate')->ne('0000-00-00 00:00:00')
    ...
    public function getNodeReviewers($approvalID)
    {
        ...
        foreach($nodeGroups as $nodeID => $nodes)
        {
            ...
            foreach($nodes as $node)
            {
                ...
                if($node->type == 'review') $nodeMap[$nodeID]['reviewers'][] = array('account' => $node->account, 'status' => $node->status, 'result' => $node->result, 'reviewedDate' => helper::isZeroDate($node->reviewedDate) ? '' : $node->reviewedDate, 'opinion' => $node->opinion);
    ...
    private function create($flowID, $reviewers, $version = 0, $objectType = '', $objectID = 0)
    {
        ...
        $parent = $this->dao->select('parent')->from(TABLE_DEPT)->where('manager')->eq($this->app->user->dept)->orderBy('grade')->fetch('parent');
    ...
    private function next($approvalID, $prev, $action = 'submit')
    {
        ...
        foreach($nodeMap as $nodeID => $users)
        {
            ...
            else
            {
                ...
                if(!empty($users['cc'])) $this->sendMessage($users['cc'], $approvalID, $nodeID, $action);
            }
        }
    }
    ...
    {
        $this->dao->update(TABLE_APPROVALNODE)->set('status')->eq('doing')->where('approval')->eq($approvalID)->andWhere('node')->eq($nodeID)->andWhere('type')->eq('review')->exec();
    ...
    private function cc($approvalID, $nodeID, $ccs, $action)
    {
        ...
        $this->sendMessage($ccs, $approvalID, $nodeID, $action);
    }
     ...
     * @return void
     */
    private function sendMessage($sendList, $approvalID, $nodeID, $method)
    {
        $approval = $this->dao->select('t1.objectType, t1.objectID, t2.type')->from(TABLE_APPROVAL)->alias('t1')->leftJoin(TABLE_APPROVALFLOW)->alias('t2')->on('t1.flow = t2.id')->where('t1.id')->eq($approvalID)->fetch();
        ...
        if(isset($messageSetting['message']))
        {
            ...
            {
                $this->saveNotice($sendList, $objectType, $objectID, $object);
            }
        }
        ...
        if(isset($messageSetting['webhook']))
        {
            ...
            if($this->app->moduleName == 'flow' and isset($actions[$objectType]) and in_array($method, $actions[$objectType]))
            {
                $this->sendWebHook($sendList, $objectType, $objectID, $object);
            }
        }
        ...
        {
            if($method == 'cc') $ccList = implode(',', $sendList);
            if($method != 'cc') $toList = implode(',', $sendList);
        }
     ...
     * @return void
     */
    public function saveNotice($sendList, $objectType, $objectID, $object)
    {
        $nameFields = $this->config->action->objectNameFields[$objectType];
        $title      = zget($object, $nameFields, '');
        $url        = helper::createLink($objectType, 'view', "id=$objectID");
        $data       = html::a($url, "[#{$objectID}::{$title}]");

        $notify = new stdclass();
        $notify->objectType  = 'message';
        $notify->action      = '0';
        $notify->toList      = ',' . implode(',', $sendList) . ',';
        $notify->data        = $data;
        $notify->status      = 'wait';
     ...
     * @return void
     */
    public function sendWebHook($sendList, $objectType, $objectID, $object)
    {
        ...
        foreach($users as $user)
        {
            if(in_array($user->account, $sendList))
            {
                ...
                foreach($webhooks as $id => $webhook)
                {
                    ...
                    $result = $this->webhook->fetchHook($webhook, $postData);
    ...
    public function pass($objectType, $object, $extra = '')
    {
        ...
        foreach($nodes as $node)
        {
            ...
            $ccs = $this->dao->select('account')->from(TABLE_APPROVALNODE)
                ->where('approval')->eq($approvalID)
                ->andWhere('node')->eq($node->node)
                ->andWhere('type')->eq('cc')
                ->fetchAll();
            ...
            $ccers = array();
            foreach($ccs as $cc) $ccers[] = $cc->account;

            $this->cc($approvalID, $node->node, $ccers, 'review');
        ...
        if(empty($doing))
        {
            $reject = $this->dao->select('*')->from(TABLE_APPROVALNODE)
                ...
                ->fetchAll();
            return $this->finish($approvalID, empty($reject) ? 'pass' : 'fail');
        }
    }
    ...
    public function reject($objectType = '', $object = null, $extra = '')
    {
        ...
        return $this->finish($approvalID, 'fail');
    }
     ...
     * @return bool
     */
    public function finish($approvalID, $result)
    {
        ...
        $nodeID = $this->dao->select('node')->from(TABLE_APPROVALNODE)
            ->where('approval')->eq($approvalID)
            ->orderBy('id_desc')
            ->fetch('node');
    ...
    public function restart($objectType, $objectID)
    {
        ...
        $nodes = $this->dao->select('*')->from(TABLE_APPROVALNODE)
            ->where('approval')->eq($oldApprovalID)
            ->fetchAll('id');
    ...
    public function cancel($objectType, $objectID)
    {
        ...
        $nodes = $this->dao->select('*')->from(TABLE_APPROVALNODE)
            ...
            ->andWhere('type')->eq('review')
            ->markRight(1)
            ->orWhere('(status')->eq('done')
            ->andWhere('title')->eq($this->lang->approvalflow->nodeTypeList['start'])
            ->andWhere('type')->eq('cc')
            ->markRight(2)
            ->fetchAll();

        $reviewers = array();
        foreach($nodes as $node) $reviewers[] = $node->account;
        ...
        $approval = $this->getByID($approvalID);
        if($reviewers) $this->sendMessage($reviewers, $approvalID, $node->node, 'cancel');
        return !dao::isError();
    }
