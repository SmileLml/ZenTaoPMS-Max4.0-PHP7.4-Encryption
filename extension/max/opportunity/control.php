...
class opportunity extends control
{
    ...
    public function edit($opportunityID, $from = 'project')
    {
        ...
        $executionList = array(0 => '');
        $executions = $this->loadModel('execution')->getByProject($opportunity->project);
        foreach ($executions as $execution) $executionList[$execution->id] = $execution->name;
    ...
    public function view($opportunityID, $from = 'project')
    {
        ...
        $this->view->approvers   = $this->loadModel('assetlib')->getApproveUsers('opportunity');
        $this->view->libs        = $this->assetlib->getPairs('opportunity');
        $this->view->executions  = array(0 => '') + $this->loadModel('execution')->getPairs($opportunity->project);
    ...
    public function export($objectID, $browseType, $orderBy)
    {
        if($_POST)
        {
            ...
            /* Get executions. */
            $executions = $this->dao->select('id, name')->from(TABLE_PROJECT)->where('id')->in(array_column($opportunities, 'execution'))
                ->fetchAll('id');
