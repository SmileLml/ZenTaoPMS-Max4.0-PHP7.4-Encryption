...
class effortModel extends model
{
     ...
     * @return int
     */
    public function create($objectType, $objectID, $consumed, $work, $extra = '')
    {
        $effort = new stdclass();
    ...
    public function batchCreate()
    {
        ...
        foreach($efforts->id as $id => $num)
        {
            ...
            {
                if($efforts->objectType[$id] == 'task' and isset($efforts->dates[$num]) and helper::isZeroDate($efforts->date) and helper::isZeroDate($efforts->dates[$num])) die(js::alert($this->lang->effort->common . $efforts->id[$id] . ' : ' . $this->lang->task->error->dateEmpty));
        ...
        $lastDatePairs  = $this->dao->select('objectID,max(date) as date')->from(TABLE_EFFORT)->where('objectID')->in($taskIDList)->andWhere('objectType')->eq('task')->andWhere('deleted')->eq(0)->fetchPairs('objectID', 'date');
    ...
    {
        $this->loadModel('action');
        $newConsumed = $this->dao->select('format(sum(consumed), 2) as consumed')->from(TABLE_EFFORT)->where('objectID')->eq($objectID)->andWhere('objectType')->eq($objectType)->andWhere('deleted')->eq(0)->fetch();
    ...
    public function getActions($date, $account, $objectType = '', $objectID = '')
    {
        ...
        $taskIdList  = $this->dao->select('id')->from(TABLE_TASK)->where('`id`')->in($taskIdList)->andWhere('deleted')->eq(0)->fetchPairs('id', 'id');
        $teams       = $this->dao->select('id,task,account')->from(TABLE_TASKTEAM)->where('task')->in($taskIdList)->fetchGroup('task', 'id');
    ...
    public function getRecentlyExecutions($status = 'all', $limit = 20, $filterID = array())
    {
        ...
        $executions = $this->dao->select('t1.id as id, t1.name as name, t2.name as project, t1.multiple')->from(TABLE_EXECUTION)->alias('t1')
