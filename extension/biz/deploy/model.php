...
class deployModel extends model
{
    ...
    public function manageScope($deployID)
    {
        ...
        foreach($data->service as $i => $serviceID)
        {
            ...
            $scope->hosts   = trim(join(',', $data->hosts[$i]), ',');
            $scope->remove  = trim(join(',', $data->remove[$i]), ',');
            $scope->add     = trim(join(',', $data->add[$i]), ',');
    ...
    public function manageStep($deployID, $stage = 'all')
    {
        ...
        if($deleteSteps) $this->dao->delete()->from(TABLE_DEPLOYSTEP)->where('id')->in($deleteSteps)->andWhere('deploy')->eq($deployID)->exec();
