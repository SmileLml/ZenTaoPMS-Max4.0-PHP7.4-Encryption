...
class deploy extends control
{
    ...
    public function finish($deployID)
    {
        ...
        if($_POST)
        {
            ...
            $this->loadModel('action')->create('deploy', $deployID, "finished", $this->post->comment);
            $this->deploy->sendmail($deployID, $actionID);
    ...
    public function assignTo($stepID)
    {
        ...
        if($_POST)
        {
            ...
            if($changes or $this->post->comment)
            {
                $actionID = $this->loadModel('action')->create('deploystep', $stepID, "Assigned", $this->post->comment);
                $this->action->logHistory($actionID, $changes);
            }
