...
class zentaobizDoc extends docModel
{
    ...
    public function diff($fromText, $toText)
    {
        ...
        /* Get diff. */
        $diffOpcodes  = FineDiff::getDiffOpcodes($fromText, $toText, FineDiff::$wordGranularity);
    ...
    public function create()
    {
        ...
        if($result and isset($result['status']) and $result['status'] == 'new' and strpos($this->config->doc->officeTypes, $this->post->type) !== false)
        {
            ...
            $doc->size       = filesize($templateFile);
            $doc->objectType = 'doc';
            $doc->objectID   = $docID;
            ...
            if(!dao::isError())
            {
                copy($templateFile, $this->file->savePath . DS . str_replace(".{$extension}", '', $doc->pathname));
                ...
                unset($_GET['onlybody']);
                $editUrl = helper::createLink('file', 'download', "fileID=$fileID&mouse=left&edit=1");
                $viewUrl = helper::createLink('doc', 'view', "docID=$docID");

                echo js::start();
                echo "window.open('{$editUrl}');\n";
                echo "parent.parent.location.href = '{$viewUrl}';\n";
                die(js::end());
            }
        }
        return $result;
    ...
    public function getFrontCatalog($nodes, $serials, $articleID = 0)
    {
        ...
        foreach($nodes as $childNode)
        {
            ...
            if($childNode->type == 'chapter')
            {
                ...
                if($this->app->getMethodName() == 'objectlibs' and common::hasPriv('doc', 'edit'))
                {
            ...
            elseif($childNode->type == 'article')
            {
                ...
                echo html::a(helper::createLink('doc', 'objectLibs', "type=book&objectID=0&libID=$childNode->lib&docID=$childNode->id"), "<i class='icon icon-file-text text-muted'></i> &nbsp;" . $serial . ' ' . $childNode->title, '', "class='item doc-title' title='{$childNode->title}'");
                if($this->app->getMethodName() == 'objectlibs' and common::hasPriv('doc', 'edit'))
                {
                    echo "<div class='tree-actions'>";
    ...
    public function getBookOptionMenu($bookID = 0, $removeRoot = false)
    {
        ...
        $treeMenu   = array();
        $stmt       = $this->dbh->query($this->buildQuery($bookID));
        $catalogues = array();
    ...
    public function buildQuery($bookID = 0)
    {
        return $this->dao->select('*')->from(TABLE_DOC)
