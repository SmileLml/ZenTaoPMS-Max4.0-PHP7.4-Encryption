        ...
        if($this->app->getModuleName() == 'report')
        {
            if(common::hasPriv('report', 'show'))
            {
                ...
                foreach($reportList as $report)
                {
                    ...
                    $reportName = !isset($name[$this->app->getClientLang()]) ? reset($name) : $name[$this->app->getClientLang()];
                    $reportName = $this->replace4Workflow($reportName);
                    foreach($modules as $module)
                    {
                        if($module) $this->lang->reportList->{$module}->lists[] = $reportName . "|report|show|reportID={$report->id}&reportModule=$module";
                    }
                }
            }
        }
