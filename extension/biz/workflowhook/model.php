                    ...
                    if(!empty($hook->formulaVars))
                    {
                        foreach($hook->formulaVars as $var)
                        {
                            ...
                            if(count($params) == 3)
                            {
                                ...
                                if(empty($childDatas[$module]))
                                {
                                    /* Fetch child datas from db. */
                                    $childFlow           = $this->workflow->getByModule($module);
                                    $childDatas[$module] = $this->flow->getDataList($childFlow, 'browse', '', '', $dataID);
                                }
                                if(empty($fieldDatas[$module][$field]))
                                {
                                    /* Store field datas. */
                                    foreach($childDatas[$module] as $childData) $fieldDatas[$module][$field][] = zget($childData, $field, 0);
                                }
                                $fieldValues = isset($fieldDatas[$module][$field]) ? $fieldDatas[$module][$field] : array();
