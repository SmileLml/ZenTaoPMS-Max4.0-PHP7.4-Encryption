...
class feedback extends control
{
    ...
    public function edit($id, $browseType)
    {
        ...
        if($_POST)
        {
            ...
            if(dao::isError()) return $this->send(array('result' => 'fail', 'message' => dao::getError()));
            $files = $this->loadModel('file')->saveUpload('feedback', $id);
            if($changes or $files)
            {
                $fileAction = '';
                if(!empty($files)) $fileAction = $this->lang->addFiles . join(',', $files) . "\n" ;
                $actionID = $this->loadModel('action')->create('feedback', $id, 'Edited', $fileAction);
    ...
    public function batchEdit($browseType)
    {
        if(isset($_POST['feedbackIDList']))
        {
            ...
            $this->view->products   = array('' => '') + $this->feedback->getGrantProducts();
            $this->view->browseType = $browseType;
    ...
    public function browse($browseType = 'wait', $param = 0, $orderBy = 'id_desc', $recTotal = 0, $recPerPage = 20, $pageID = 1)
    {
        ...
        $productViewID = $param != 'all' ? $productID : 0;
    ...
    public function view($feedbackID, $browseType = '')
    {
        ...
        $this->session->set('bugList',   $uri . '#' . $this->app->tab, 'qa');
        $this->session->set('storyList', $uri . '#' . $this->app->tab, 'product');
        $this->session->set('taskList',  $uri . '#' . $this->app->tab, 'execution');
        ...
        $this->view->product     = $this->dao->select('*')->from(TABLE_PRODUCT)->where('id')->eq($feedback->product)->fetch('name');
    ...
    public function comment($feedbackID, $type = 'commented')
    {
        if($_POST)
        {
            ...
            if($type == 'asked' or $type == 'replied')
            {
                ...
                if($type == 'replied') $this->app->methodName = 'reply';
                if($type == 'asked')   $this->app->methodName = 'ask';
            }
    ...
    {
        if(empty($_POST['feedbackIDList'])) return print(js::locate($this->session->feedbackList, 'parent'));

        $feedbackIDList = $this->post->feedbackIDList;
        $feedbackIDList = array_unique($feedbackIDList);
        $allChanges  = $this->feedback->batchChangeModule($feedbackIDList, $moduleID);
        if(dao::isError()) return print(js::error(dao::getError()));
        foreach($allChanges as $feedbackID => $changes)
        {
            $actionID = $this->loadModel('action')->create('feedback', $feedbackID, 'Edited');
            $this->action->logHistory($actionID, $changes);
        }
    ...
    public function admin($browseType = 'wait', $param = 0, $orderBy = 'editedDate_desc,id_desc', $recTotal = 0, $recPerPage = 20, $pageID = 1)
    {
        ...
        if(!$this->config->URAndSR) unset($this->lang->feedback->featureBar['admin']['more']['touserstory']);
    ...
    public function adminView($feedbackID, $browseType = '')
    {
        $feedback = $this->feedback->getById($feedbackID);
        $this->session->set('todoList', $this->app->getURI(true), 'feedback');
        $this->session->set('feedbackProduct', $feedback->product, 'feedback');
    ...
    public function products($recTotal = 0, $recPerPage = 20, $pageID = 1)
    {
        $this->app->loadClass('pager', $static = true);
        $pager = pager::init($recTotal, $recPerPage, $pageID);
        $users = $this->loadModel('user')->getHasFeedbackPriv();
        ...
        $productStats     = $this->product->getStats($orderBy, $pager, 'noclosed', '', 'story');
        ...
        $this->view->feedbackView     = $this->feedback->getFeedbackView(array_keys($productStats));
    ...
    public function manageProduct($productID)
    {
        ...
        if(empty($view))
        {
            $hasPrivUsers = $this->dao->select('DISTINCT t1.account')->from(TABLE_FEEDBACKVIEW)->alias('t1')
                ->leftJoin(TABLE_PRODUCT)->alias('t2')->on('t1.product=t2.id')
                ->where('t2.deleted')->eq('0')
                ->fetchPairs('account', 'account');
            foreach($users as $account => $user)
            {
                if(isset($hasPrivUsers[$account])) continue;
     ...
     * @return void
     */
    public function syncProduct($productID = 0, $module = 'feedback')
    {
    ...
    public function ajaxGetProjects($productID = 0, $field = 'bugProjects', $onchange = '')
    {
        ...
        $projects = $this->loadModel('product')->getProjectPairsByProduct($productID, $branches);
    ...
    public function export($browseType, $orderBy)
    {
        if($_POST)
        {
            ...
            /* Get feedbacks. */
            $feedbacks = $this->dao->select('*')->from(TABLE_FEEDBACK)->where($this->session->feedbackQueryCondition)
                ->beginIF($this->post->exportType == 'selected')->andWhere('id')->in($this->cookie->checkedItem)->fi()
                ->orderBy($sort)->fetchAll('id');

            $productIdList = $bugIdList = $storyIdList = $todoIdList = $taskIdList = array();
            foreach($feedbacks as $feedback)
            {
            ...
            $users    = $this->loadModel('user')->getPairs('noletter');
            $modules  = $this->loadModel('tree')->getOptionMenu(0, $viewType = 'feedback', $startModuleID = 0);
        ...
        $fileName = zget($this->lang->feedback->tabList, $browseType, '');
    ...
    public function productSetting()
    {
        ...
        foreach($productList as $productID => $product) $productHeadMap[$productID] = array('feedback' => $product->feedback, 'ticket' => $product->ticket);
